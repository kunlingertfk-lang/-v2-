...
#include <set>
#include <vector>
#define DEFAULT_LABEL_NAME_TXT_PATH "./labels.txt"

static std::vector<char*> labels;
static int g_obj_class_num = OBJ_CLASS_NUM;
static float g_box_threshold = BOX_THRESH;
...
{
    printf("load lable %s\n", locationFilename);
    return readLines(locationFilename, label, g_obj_class_num) < 0 ? -1 : 0;
}
...
int init_post_process(const char* label_path, int class_num, float box_threshold)
{
    deinit_post_process();

    g_obj_class_num = class_num > 0 ? class_num : OBJ_CLASS_NUM;
    g_box_threshold = box_threshold > 0.0f ? box_threshold : BOX_THRESH;
    labels.assign(g_obj_class_num, nullptr);

    const char *effective_label_path = (label_path != nullptr && label_path[0] != '\0')
        ? label_path
        : DEFAULT_LABEL_NAME_TXT_PATH;
    const int ret = loadLabelName(effective_label_path, labels.data());
    if (ret < 0)
    {
...
float get_box_threshold()
{
    return g_box_threshold;
}

char *coco_cls_to_name(int cls_id)
{

    if (cls_id < 0 || cls_id >= g_obj_class_num)
    {
        return "null";
    }

    if (cls_id < static_cast<int>(labels.size()) && labels[cls_id])
    {
