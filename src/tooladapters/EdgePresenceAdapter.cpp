...
EdgePresenceHalconConfig toHalconConfig(const ToolConfig &config)
{
    const QJsonObject params = config.params;
    const QJsonObject judgeRule = config.judgeRule;

    EdgePresenceHalconConfig halconConfig;
...
ToolResult makeEdgePresenceError(const ToolConfig &config,
                                 const QString &status,
                                 const QString &message)
{
    ToolResult result;
    result.toolId = config.toolId;
    result.toolType = ToolType::EdgePresence;
...
bool EdgePresenceAdapter::supports(ToolType type) const
{
    return type == ToolType::EdgePresence;
}

ToolResult EdgePresenceAdapter::run(const ToolRequest &request)
{
    const ToolConfig &config = request.config;
    if (config.toolType != ToolType::EdgePresence) {
        return makeEdgePresenceError(config,
