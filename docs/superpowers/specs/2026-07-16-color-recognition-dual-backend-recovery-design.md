...
- `ReferenceImageProvider`现在根据最终保存的 Mat 生成运行时元数据；三/四通道和 Mono 来源统一为 BGR，8/16 位深保持不变。
- `SchemeStore`使用 `IMREAD_UNCHANGED`恢复参考 PNG，避免重启后把 16 位参考图降为 8 位，并回写与快照 Mat 一致的元数据。
- 模板重开后自动选择首个有效 ROI 并校验保存的训练 Mat/输入元数据；右侧视图只显示当前会话完整外部图、基准图或相机图，并按 `roiNormalized` 叠加 ROI，禁止用持久化 ROI 裁剪图替换完整画布。完整来源图缺失时显示“当前无图像”，ROI 裁剪图仍仅供缩略图和 HSV/GMM 后端重建使用。外部完整图片不持久化，添加/删除 ROI 时不得清空本次会话的完整采样图。
- HSV 模式新增独立状态、按钮附近反馈和“重新提取 HSV 特征”按钮；批量重建全部成功后一次提交，失败不产生混合签名。
- 切换后端只刷新目标状态，不自动训练、不清除另一后端模型；HSV/GMM 保存前分别校验活动状态。
- 样本数量变化会同时刷新 HSV/GMM 操作按钮，GMM empty 分支不再跳过按钮刷新。
- 检测状态栏现在显示稳定 status，并针对 HSV stale、GMM stale 和输入格式不一致给出中文处理建议。
- HSV/GMM 错误 payload 增加声明 pixelFormat、实际 Mat depth 和通道数。
- 已通过 GMM B2 lifecycle、GMM build、GMM production detection、HSV Phase A，以及主工程 qmake/make；专项测试覆盖参考图 BGR8/BGR16 元数据、重开样本选择、HSV 事务式重建和双后端状态隔离。
