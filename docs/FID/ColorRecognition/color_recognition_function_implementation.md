...
- `src/ToolsDialog.cpp`
  - 接入颜色识别配置对话框。
  - 清理工具页残留交互 ROI。
  - 工具页/主览窗口应保留检测运行产生的 `ROI` 结果 overlay。
...
### 6. 工具页显示检测 ROI，清理残留交互 ROI

用户需求：

- 在工具页面/主览窗口显示颜色识别测试结果时，图像上必须显示当前检测 ROI 框。
- 不得把 runner 输出的 `ROI` 结果 overlay 当作配置残留过滤掉。
- 只清理配置阶段遗留的交互式编辑 ROI、样本 ROI 预览或屏蔽区编辑态图形。
- 当前曾增加工具页预览过滤和 `clearRoi()`，后续需要改为只清理交互残留，并在 GUI 里手动确认：
  - 从颜色识别配置页返回工具页。
  - 切换工具页选中颜色识别工具。
  - 图像上不应残留橙色配置编辑 ROI。
  - 若该工具已有测试结果 overlay，图像上应显示检测 ROI 框、OK/NG、类别和得分。
...
- 测试运行中检测 ROI 可继续拖动：
  ...
  - 连续测试中如果存在交互式检测 ROI，可避免重复绘制同一 ROI；但工具页/主览窗口展示最终结果时必须保留 runner 输出的 `ROI` 检测框。
