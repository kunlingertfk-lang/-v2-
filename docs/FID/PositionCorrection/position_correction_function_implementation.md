...
- 基准图位置修正固定 ID 为 `reference.positionCorrection`，界面显示为
  `0 基准图.位置修正信息`。
- 工具级位置修正可订阅合法前置模板定位，也可订阅已开启且已创建的基准图位置修正；
  X/Y/角度来自同一来源，尺度随同一来源隐式读取。
- 工具级位置修正不拥有模板 ROI、不执行模板匹配；旧工具级模板 ROI 字段保存时迁移删除。
- `ToolEngine` 每帧清空动态结果表；位置修正生产者和消费者校验稳定 ID、类型与 `frameId`，
  不允许跨帧沿用姿态或矩阵。
- 方案级与工具级姿态字段执行严格有限数值校验；非法 X/Y/角度返回
  `invalid_reference_pose` 或 `invalid_run_pose`，非法尺度返回 `invalid_pose_scale`。
- 当前完整消费者为颜色比较和 Blob 有无。其他工具链尚未接入，不在本轮范围。

本文后续按日期保留历史实施记录。标有旧日期的“当前状态”“尚未实现”和阶段边界只描述
当时事实；与本节冲突时以本节和当前代码为准。
...
- 未配置、来源失效、消费者尚未接入和 HALCON 异常时返回明确状态。
...
- 本段撰写时基准图私有模板定位器尚未完成；后续记录和当前代码已经完成该链路。
- 本段撰写时下游尚未应用矩阵；当前颜色比较与 Blob 有无已经完整消费。
- 本段撰写时工具级测试仍使用基准图；当前已经改用真实当前帧。
...
- 当时基准图位置修正只作为下游修正来源；当前已允许工具级位置修正订阅其
  X/Y/角度/尺度。
...
### 2026-07-26 - 基准图私有模板屏蔽区域

- `ReferenceImageDialog` 的基准图位置修正增加矩形、圆形和多边形屏蔽区，以及清除和完成操作；三种绘制方式互斥，屏蔽区必须在模板 ROI 确认后编辑。
- 方案级 `ReferencePositionCorrectionConfig` 升级为版本 3，保存屏蔽类型及归一化矩形、多边形或圆心/半径；旧方案缺少字段时按无屏蔽兼容。
- 创建基准和基准图自匹配时，屏蔽配置传入私有 `TemplateLocationHalconRunner`；运行态由 `ToolEngine` 将同一配置传给基准图私有定位器，确保基准姿态与运行姿态使用相同建模 Domain。
- 修改或清除屏蔽区会使已创建的基准姿态失效，必须重新创建基准；屏蔽区无效、与模板无交集或屏蔽后有效面积不足时沿模板定位错误合同返回明确错误。
- 已通过 `position_correction_smoke`、`reference_position_correction_private_model_smoke`、`position_correction_engine_context_smoke` 和主工程 qmake/make；专项 smoke 覆盖方案序列化、基准自匹配及运行态配置消费。
