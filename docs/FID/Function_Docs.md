...
- 所有 `FrameViewHelper` 主画布统一通过公共像素探查能力显示原图坐标和像素值，不得在单个 Dialog 中重复读取 viewport 坐标或自行换算缩放比例。
- 坐标原点为图像左上角 `(0,0)`，X 向右、Y 向下；必须使用 `view -> scene -> 原图整数像素` 链路，缩放和平移前后同一像素的坐标和值保持不变。
- 鼠标位于图像外黑边、离开画布或图像为空时显示无效占位，不得把位置钳制到图像边缘。
- 彩色图显示 R/G/B，灰度图显示 Gray；数值以 `FrameViewHelper` 当前持有的 `QImage` 为准，不得将上游已降位深的显示图描述成相机原始数据。
- 像素信息必须使用独立状态标签，不得覆盖工具运行状态、错误信息、OK/NG 或耗时；静态缩略图默认不启用像素探查。
